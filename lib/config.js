...
function amtDevice(deviceid, body) {
  ...
  return {
    ...
    // Where the operating system answers. AMT has its own address on this
    // hardware, so the OS cannot be probed at `host`.
    // A dual-boot box has one address per OS, so this is a list.
    osHosts: (function () {
      const raw = body.oshost || (entry && (entry.osHosts || entry.osHost)) || null;
      if (raw == null) return [];
