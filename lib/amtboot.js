...
async function probeOs(host) {
  const evidence = await Promise.all(
    OS_PORTS.map(async (p) => ({ port: p.port, service: p.service, open: await probePort(host, p.port) })));
  const openSet = new Set(evidence.filter((e) => e.open).map((e) => e.port));
  let os = 'none';
  for (const p of OS_PORTS) { if (openSet.has(p.port)) { os = p.decides; break; } }
  return {
    os: os,
    reachable: openSet.size > 0,
    openPorts: evidence.filter((e) => e.open).map((e) => e.service + '/' + e.port),
    evidence: evidence
  };
}
